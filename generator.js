...
module.exports = (api, options, rootOptions) => {
  ...
  } else if (options.template === 'default-middle') {
    api.extendPackage(pkg => {
      return {
        dependencies: {
          'luch-request': '^2.0.1',
  ...
  api.render(async function (files) {
    ...
    } else if (template === 'default-middle') {
      // 新增加入中间件的模板
      await generate(path.resolve(__dirname, './template/default-middle'), files, base, rootOptions)
    } else {
